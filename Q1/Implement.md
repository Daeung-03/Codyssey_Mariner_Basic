...
- `--security-opt apparmor=unconfined`: reduces container security restrictions
  that can interfere with firewall testing.
...
Checked whether cron was already installed:

docker exec agent-q1 bash -lc 'command -v cron || command -v crond || true; command -v crontab || true'

No cron or crontab binary was found.
